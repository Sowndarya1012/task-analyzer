...
## Overview
The Task Priority Analyzer is a smart productivity tool that ranks tasks using a weighted scoring algorithm. The goal of the tool is to help users decide what they should work on right now, given multiple factors such as urgency, importance, effort, and dependencies.
...
6. Set up the frontend using a simple HTTP server (in a new terminal)
```bash
cd frontend
...
All these components are then combined into a final weighted score. The **Smart Balance** strategy uses a balanced weighting across all components. Other strategies modify weights, for example:
...
- Drag & drop UI
- Improve scoring via user feedback (learning system)
